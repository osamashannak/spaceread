...
func (s *Server) ListReviews() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		reviews, total, err := s.db.ListReviews(r.Context(), admindb.ListReviewOptions{
			...
			HasFingerprint:       parseChoiceQuery(r, "has_fingerprint", "any", "any", "has", "none"),
			...
			ThumbmarkFingerprint: strings.TrimSpace(r.URL.Query().Get("thumbmark_fingerprint")),
			CreepFingerprint:     strings.TrimSpace(r.URL.Query().Get("creep_fingerprint")),
